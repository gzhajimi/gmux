...
`prefix+q` 是所有「区域操作」（`prefix+f / z / g / c / x / d`）的前置步骤：先把 region 编号画到屏上，按数字选中一个，后续区域操作就对它生效。也可以在 `prefix+q` 之后**直接按操作键再按数字**，一个 chord 内就对指定区执行该操作（见用法 2）。
...
### 用法 2：选区即执行（q+动作+数字，一步到位）

不想「先选区、之后再操作」两步走的话，在 `prefix+q` 之后**先按操作键、再按数字**，一个 chord 内选中该区并立刻执行该操作：

```
按  prefix
按  q              ← 屏上每个 region 显示编号
按  z              ← 选定要执行的操作（这里 z = 全屏）
按  3              ← 选中 region 3 并立即全屏
```

- 可作为操作键的就是六个区域动作的 chord（用各自**当前的绑定键**，改绑后同步）：
  `g` 聚焦 · `f` 轮换窗口 · `z` 全屏 · `d` 回收 · `c` split · `x` 关闭。
- 执行后该区**保持选中**，后续裸 `prefix+f / z / g / c / x / d` 继续对它生效。
- 编号 ≥ 10 时同样用第二个 `q` 进多位模式：`prefix` → `q` → `z` → `q` → `1` `2` → `Enter`（或停手超时提交）。
- 紧跟在 `q` 之后的若是数字（而非操作键），仍是用法 1 的「仅选中」，行为不变。
- `Esc` 取消。若按的操作键被禁用（在设置里清空了该动作的快捷键），它不算操作键，按下即取消并透传。

### 用法 3：两位数选区（region 编号 ≥ 10）
