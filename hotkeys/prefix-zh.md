...
3. 等待期间收到的下一键决定下一步：
   ...
   - 是某个绑定的 key（`0-9`、`a-z`、`F1`–`F24`、命名键或 OEM 标点）→ 触发该绑定。
   - 是 `Esc` → 静默取消。
...
- `prefix` `<绑定键>` → 触发该绑定。合法绑定键：数字 `0-9`、字母 `a-z`、功能键 `F1`–`F24`、命名键（`Space`、`Enter`、`Tab`、方向键、`Home`/`End`、`PageUp`/`PageDown`、`Insert`、`Delete`、`Backspace`）、OEM 标点（`;` `/` `` ` `` `[` `\` `]` `'` `,` `.` `-` `=`），共 85 个。
