...
「绑定」= 一个**快捷键** + 描述 + 一组预案。绑定的 key 可以是按键选择器里的任意单键 —— 数字 `0-9`、字母 `a-z`、功能键 `F1`–`F24`、命名键（`Space`、`Enter`、`Tab`、方向键、`Home`/`End` 等）或 OEM 标点（`;` `/` `[` `]` 等），共 85 个。（数字和字母默认展开；功能键、命名键、标点在选择器的折叠分区里。）按 `prefix + <key>` 就触发它（默认 prefix 是 `Ctrl+M`，所以是 `Ctrl+M 1` / `Ctrl+M a` 这种形式）。
...
| 按键序列 | 作用 |
|---|---|
| `prefix` `<绑定键>` | 触发 `bindings.<key>`（`0-9`、`a-z`、`F1`–`F24`、命名键或 OEM 标点）—— 引擎挑一个匹配的预案、把窗口摆到位 |
